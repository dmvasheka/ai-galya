    ...
    <div className="space-y-4">
      ...
      {type === "single" ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input type="date" value={start} onChange={e => setStart(e.target.value)} className="border rounded-xl px-3 py-2 w-full" />
          <input type="date" value={end} onChange={e => setEnd(e.target.value)} className="border rounded-xl px-3 py-2 w-full" />
        </div>
      )}
      ...
      <button
        ...
        onClick={() => onSubmit({ type, date, start, end, language, theme, uploadToDrive: upload })}
      >
        Generate forecast
      </button>
    </div>
